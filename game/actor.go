...
func (p Policy[Ac]) ValidateForLegalActions(legalActions []Ac, checkUnique bool) error {
	if checkUnique {
		if !slicesx.IsUnique(legalActions) {
			return errors.New("legalActionsが重複しています")
		}
	}

	if len(legalActions) == 0 {
		return errors.New("legalActionsが空です: len(legalActions) > 0 であるべき")
	}
	...
	if sum == 0 {
		return errors.New("policyの確率の合計が0です: 合計は正であるべき")
	}
	return nil
}
...
func MaxSelectFunc[Ac, Ag comparable](policy Policy[Ac], agent Ag, rng *rand.Rand) (Ac, error) {
	if len(policy) == 0 {
		var zero Ac
		return zero, errors.New("policyが空です: len(policy) > 0 であるべき")
	}
