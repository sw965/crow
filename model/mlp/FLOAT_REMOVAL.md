...
`model/mlp/binary` の学習カーネルから浮動小数を取り除くための作業手順です。
...
> 根拠となる実験結果は [binary/glulab/](binary/glulab/) にあります。
...
| ファイル | 対応 |
|---|---|
| `binary/pairwise.go:111` | `t.Margin` → `t.PairwiseMargin` |
| `examples/mnist/main.go`、`nnlab/bepreadout/main.go` | `trainer.Margin = 0.5` → `trainer.LogitMargin = 0.5` |
| `binary/margin_internal_test.go`（新規） | `minPrototypeHammingDistance` のテスト |
| `binary_test.go` の `TestTrainerValidate` | `LogitMargin` の範囲外を弾くことのテストを追加 |
| `binary_test.go` | `0.5` → `marginBits = 2`（元コメントの `8 × 0.5 / 2 = 2` のとおり） |
...
[binary/PROTOTYPES.md](binary/PROTOTYPES.md) B-3 と同じ結論。
...
* [layer.go](binary/layer.go) の `Dense.Update`: `lr float32` → `lrHalfPow int`。
...
* [train.go](binary/train.go): `LR float32` → `LRHalfPow int`（既定値 3 = 1/8）。
  `Validate` は `LRHalfPow < 0` を弾く（0 は「毎回更新」として有効）。
* `binary_test.go`: `TestTrainerValidate` の `LR = 0.0` → `LRHalfPow = -1`。
...
### 削除するもの（`model/mlp/binary/layer.go`）
...
この恒等式は [binary/glulab/main_test.go](binary/glulab/main_test.go) の
...
| ファイル | 内容 |
|---|---|
| `binary/biaslab/main_test.go:364` | `mine.gateScale = ctx.GateDropThresholdScale` → **コンパイルエラー** |
...
順伝播（[layer.go:123-142](binary/layer.go)）:
...
| ファイル | 内容 |
|---|---|
| `binary/biaslab/main_test.go:349` | `ctx.NoiseStdScale = 0` → `NoiseHalfWidth = 0`（差分テストを決定的にするため） |

ラボの `-noise` フラグ（`biaslab/main.go:1122` 等）はラボ独自なのでコンパイルは通ります。

### 補足: CP+R について

[binary/REPORT.md](binary/REPORT.md) の差分5に、論文のCP+R強化ステップを省略した理由として
...
`model/mlp/binary/*.go`（ラボを除く）に残っているのは次だけで、いずれも残す判断をした。
...
```powershell
Select-String -Path model/mlp/binary/*.go -Pattern "float32|float64|math\.Sqrt|math\.Round|math\.Abs|math\.Log|math\.Exp|Float32\(\)|Float64\(\)"
```
