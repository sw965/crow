...
// bep.SatisfiesUpdateCriterion と完全に一致する事を確認する(再実装の忠実性検証)。
func TestSatisfiesUpdateCriterionMatchesCrow(t *testing.T) {
	...
	for _, margin := range []float32{0.0, 0.01, 0.1, 0.5, 1.0} {
		for trial := range 50 {
			...
			want, err := bep.SatisfiesUpdateCriterion(y, label, protos, marginBits)
			if err != nil {
				t.Fatalf("予期せぬエラー: %v", err)
			}
			if got != want {
...
func newPairedDenses(t *testing.T, wRows, wCols int, seed uint64) (*bep.Dense, *dense) {
	t.Helper()
	crowDense, err := bep.NewDense(wRows, wCols, rand.New(rand.NewPCG(seed, seed+1)))
	if err != nil {
		t.Fatalf("予期せぬエラー: %v", err)
	}
