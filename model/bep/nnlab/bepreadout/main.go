// bepreadout: 既存の crow/model/bep (BEP) の読み出しだけを差し替えると
...
func buildActCache(m *bep.Model, xs bitsx.Matrices, labels []int, workers int) (*actCache, error) {
...
func main() {
	...
	trainer, err := bep.NewTrainer(&model, workers)
	if err != nil {
		log.Fatal(err)
	}
