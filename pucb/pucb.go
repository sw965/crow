...
// Calculator は並列環境で安全に動作するよう atomic 操作を用いています。
type Calculator struct {
	Func         Func
	P            float32
	VirtualValue float32
	...
	// Lock-free updates
	wBits  atomic.Uint32 // float32をビット列として保存
...
func (c *Calculator) AddW(v float32) error {
	...
	// CAS (Compare-And-Swap) を用いた lock-free な float32 加算
	for {
		oldBits := c.wBits.Load()
		oldW := math.Float32frombits(oldBits)
		newW := oldW + v
		newBits := math.Float32bits(newW)
		if c.wBits.CompareAndSwap(oldBits, newBits) {
			break
		}
	}
	return nil
...
func (c *Calculator) DecrementO() error {
	for {
		...
		if c.o.CompareAndSwap(old, old-1) {
			break
		}
	}
	return nil
...
// Snapshot はデータの一貫性を保つため、現在の値を一度に取得します。
func (c *Calculator) Snapshot() (w float32, visits int, o int) {
	return math.Float32frombits(c.wBits.Load()), int(c.visits.Load()), int(c.o.Load())
}

func (c *Calculator) Visits() int {
	return int(c.visits.Load()) + int(c.o.Load())
}

func (c *Calculator) W() float32 {
	w, _, o := c.Snapshot()
	return w + (c.VirtualValue * float32(o))
}

func (c *Calculator) Q() float32 {
	w, visits, o := c.Snapshot()
	totalVisits := visits + o
	if totalVisits == 0 {
		return 0.0
	}
	return (w + (c.VirtualValue * float32(o))) / float32(totalVisits)
}
...
func (c *Calculator) U(sumVisits int) (float32, error) {
	...
	// Lock-free 環境下では、親ノードのsumVisitsを読んだ直後に他のスレッドが子ノードのvisitsを更新し、
	// sumVisits < totalVisits になる瞬間があり得るため、下限を補正します。
	if sumVisits < totalVisits {
		sumVisits = totalVisits
	}

	var q float32
	if totalVisits > 0 {
		q = (w + (c.VirtualValue * float32(o))) / float32(totalVisits)
	}

	u := c.Func(q, c.P, sumVisits, totalVisits)

	if v := float64(u); math.IsNaN(v) || math.IsInf(v, 0) {
		return 0.0, fmt.Errorf(
			"Funcの戻り値が不正(NaN/Inf): q=%.6g P=%.6g sumVisits=%d visits=%d u=%.6g",
			q, c.P, sumVisits, totalVisits, u,
		)
	}
	return u, nil
}
