...
func (node *Node[S, A])SelectAndExpansion(allNodes Nodes[S, A], f *FnCaller[S, A], c float64, r *mrand.Rand, cap int) (S, Nodes[S, A], Selects[S, A], error) {
	selects := make(Selects[S, A], 0, cap)
	state := node.State

	for {
		action := orand.Choice(node.PUCBManager.MaxKeys(c), r)
...
func (nodes Nodes[S, A]) Find(state *S, eq StateEqualFn[S]) (*Node[S, A], error) {
