...
// Edge は特定の行動に対するノード展開を保護する極小スコープの構造体です。
type Edge[S any, M, A comparable] struct {
	sync.Mutex
	NextNodes Nodes[S, M, A]
}

// Node から Mutex を排除し、完全な Lock-free 読み出しを実現しました。
type Node[S any, M, A comparable] struct {
	State           S
	Agent           A
	virtualSelector pucb.VirtualSelector[M]
	edgesByMove     map[M]*Edge[S, M, A]
}
...
func (ss selectBuffers[S, M, A]) backward(evals LeafNodeEvalByAgent[A]) {
	for _, s := range ss {
		...
		// NodeのLockなしで直接アトミック操作を実行
		calc := node.virtualSelector[move]
		calc.DecrementO()
		calc.AddW(eval)
		calc.IncrementVisits()
	}
}
...
func (e Engine[S, M, A]) NewNode(state S) (*Node[S, M, A], error) {
	...
	if len(legalMoves) == 0 {
		return nil, fmt.Errorf("NewNode: legalMoves is empty")
	}
	...
	s := make(pucb.VirtualSelector[M], len(legalMoves))
	edges := make(map[M]*Edge[S, M, A], len(legalMoves))

	for _, move := range legalMoves {
		p := policy[move]
		s[move] = &pucb.Calculator{Func: e.PUCBFunc, P: p, VirtualValue: e.VirtualValue}
		edges[move] = &Edge[S, M, A]{} // 枝ごとに初期化
	}
	...
	if !found {
		return nil, fmt.Errorf("NewNode: invalid agent")
	}
...
// 引数に capacity ではなく、ワーカー専用の buffer ポインタを受け取るように変更
func (e Engine[S, M, A]) SelectExpansionBackward(node *Node[S, M, A], buffer *selectBuffers[S, M, A], rng *rand.Rand) (LeafNodeEvalByAgent[A], error) {
	state := node.State
	var err error
	var isEnd bool
	...
	for {
		...
		node.virtualSelector[move].IncrementO()
		*buffer = append(*buffer, selectBuffer[S, M, A]{node: node, move: move})
		...
		var expand bool
		edge := node.edgesByMove[move] // safe concurrent read

		// ノード全体のロックではなく、この特定の手（Edge）だけをロックする
		edge.Lock()
		nextNode, ok := edge.NextNodes.FindByState(state, e.Game.Logic.EqualFunc)
		edge.Unlock()
		...
		if ok {
			...
			edge.Lock()
			if nn, ok := edge.NextNodes.FindByState(state, e.Game.Logic.EqualFunc); ok {
				nextNode = nn
				expand = false
			} else {
				edge.NextNodes = append(edge.NextNodes, newNode)
				nextNode = newNode
				expand = true
			}
			edge.Unlock()
...
func (e Engine[S, M, A]) Search(rootNode *Node[S, M, A], n int, workerRngs []*rand.Rand) (RootNodeEvalByAgent[A], error) {
	...
	p := len(workerRngs)
	rootEvalsPerWorker := make([]RootNodeEvalByAgent[A], p)
	// ワーカーごとにバッファを1度だけ事前割り当て（GC負荷ゼロの要）
	workerBuffers := make([]selectBuffers[S, M, A], p)

	for i := range p {
		rootEvalsPerWorker[i] = RootNodeEvalByAgent[A]{}
		workerBuffers[i] = make(selectBuffers[S, M, A], 0, 64) // オセロの深さをカバーする余裕を持たせる
	}

	err := parallel.For(n, p, func(workerId, idx int) error {
		rng := workerRngs[workerId]
		buf := &workerBuffers[workerId]
		
		// 容量（capacity）は維持したまま、長さ（length）を0に戻す
		*buf = (*buf)[:0]

		leafEvals, err := e.SelectExpansionBackward(rootNode, buf, rng)
		if err != nil {
			return err
		}
